...
typedef struct {
    uint32_t flowId;        // 流的唯一标识符，用于区分不同的数据流
    uint32_t src;           // 源节点的 ID，表示流的起始位置
    uint32_t dst;           // 目标节点的 ID，表示流的目的地
    double fat;             // 流的到达时间 (Flow Arrival Time)，通常以秒为单位
    double size;            // 流的大小，单位可以是字节 (Bytes) 或者其他大小单位
    uint32_t idx;           // 流的索引，在数组或列表中的位置
    double remaining_size;  // 流的剩余大小，表示当前还未传输完成的数据量
    double fct_est;         // 流的估计完成时间 (Estimated Flow Completion Time)
    float bw_bottleneck;    // 流经过的瓶颈链路的带宽，单位可能是 Gbps 或 Mbps
    float sldn;             // 流的 Slowdown 指标，表示流完成时间的相对延迟
} Flow;
...
int main(int argc, char *argv[]) {
    ...
    /*
    检查命令行参数是否符合要求（至少 4 个）。
    从命令行提取检查点路径 (ckpt_path_llama、ckpt_path_mlp) 和数据路径 (data_path)。
    构造输入文件路径 flows.txt 和输出文件路径 fct_mlsys.txt
    */
    ...
    //"./run ../ckpts/model_llama{model_suffix}.bin ../ckpts/model_mlp{model_suffix}.bin {data_dir} -b 10
    //              -e 576 -n {n_hosts} -t 1 -f {bfsz} -k {window} -p {enable_pfc} -c {cc} -x {param_1} -y {param_2} > {data_dir}/output.txt 2>&1"
    //
    // ./run 
