...
static void pl_compute_baseL_one_layer(int l)
{
  totPE = l;        // 终端（PE）节点数量
  totNode = l;      // 总节点数量
  totLink = l - 1;  // 链路数量（每个节点之间有一条链路，数量为节点数减一）
  totRack = l;      // 机架数量（与节点数量相等）
  n_pe_per_rack = 1; // 每个机架的终端节点数

}
/*
初始化两层拓扑中的路由表
为每个机架对之间建立链路，并分配相应的上行和下行链路
在内部，函数为每一对源和目标机架之间的流分配链路，
并使用 sdid_to_linkid 和 linkid_to_sdid 来存储映射关系。
对于链路数目较大的两层拓扑，链路的分配是通过遍历源和目标机架之间的距离（abs(dst_rack - src_rack)）来进行的
*/
...
/*
由于单层拓扑中没有分层结构，链路分配直接基于源和目标机架之间的距离。
链路分配更加简单，直接将机架间的距离用作链路标识
*/
