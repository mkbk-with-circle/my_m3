...
'''
当网络中有 3 个主机 时，主机之间的链路传播延迟都为 0
当网络中有 5 个主机 时，主机 2 到主机 3 之间的传播延迟为 1 * DELAY_PROPAGATION_BASE，
    而其他主机之间的传播延迟为 0。
当网络中有 7 个主机 时
    主机 2 到主机 3 之间的传播延迟为 1 * DELAY_PROPAGATION_BASE。
	主机 2 到主机 4 之间的传播延迟为 2 * DELAY_PROPAGATION_BASE。
	主机 3 到主机 4 之间的传播延迟为 1 * DELAY_PROPAGATION_BASE。
	其他主机之间的传播延迟为 0。
'''
LINK_TO_DELAY_DICT={
    3:np.array([0,0,0]),
...
def get_base_delay_pmn(sizes, n_links_passed, lr_bottleneck, flow_idx_target, flow_idx_nontarget_internal):
    # 计算每个数据包的头部大小（剪裁到最大为 MTU，最小为 0）
    pkt_head = np.clip(sizes, a_min=0, a_max=MTU)

    # 计算传播延迟，延迟是基于经过的链路数量计算的
    delay_propagation = DELAY_PROPAGATION_BASE * n_links_passed

    # 计算数据包大小，包括头部和报文头大小，转换为比特
    pkt_size = (pkt_head + HEADER_SIZE) * BYTE_TO_BIT

    # 计算传输延迟
    delay_transmission = (
        np.multiply(pkt_size / lr_bottleneck, flow_idx_target)   # 目标流的传输延迟
        + pkt_size / (lr_bottleneck * 4) * (n_links_passed - 2)  # 非目标内部流的延迟修正
        - np.multiply(pkt_size / lr_bottleneck, flow_idx_nontarget_internal)  # 非目标流的传输延迟减少
    )
